...
** =============================================================================
version 12.1
use "https://github.com/davetannenbaum/medical-partitioning/blob/master/raw%20data.dta?raw=true", clear
...
generate dv1a = inrange(acute_nonstrep1,1,3)
generate dv1b = inrange(acute_nonstrep2,1,3)
generate dv1c = inrange(acute_nonstrep3,1,3)
generate dv2a = inrange(acute_bronchitis1,1,4)
generate dv2b = inrange(acute_bronchitis2,1,4)
generate dv2c = inrange(acute_bronchitis3,1,4)
generate dv3a = inrange(acute_nasopharyngitis1,1,4)
generate dv3b = inrange(acute_nasopharyngitis2,1,4)
generate dv3c = inrange(acute_nasopharyngitis3,1,4)
...
generate dv4 = inrange(otitis_media,1,7)
generate dv5 = inrange(urinary,1,5)
generate dv6 = inrange(sinusitis,1,8)
generate dv7 = inrange(cellulitis,1,6)
...
// merging data and additional recoding
merge 1:1 subject_id using "https://github.com/davetannenbaum/medical-partitioning/blob/master/provider%20network%20data.dta?raw=true", gen(responder)
replace responder = responder - 2
label define responderl 0 "nonresponders" 1 "responders"
label val responder responderl
encode degree, gen(newdegree)
drop degree
rename newdegree degree
recode degree (2 3 = 1 ) (5 6 = 2) (1 4 = 3)
label define degreel 1 "MD" 2 "PA" 3 "NP"
label val degree degreel
encode specialty, gen(newspecialty)
drop specialty
rename newspecialty specialty
...
encode practice_type, gen(newpractice_type)
drop practice_type
rename newpractice_type practice_type
